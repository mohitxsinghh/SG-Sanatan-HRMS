...
// Annual quota per leave type. "Unpaid" is intentionally excluded -
// it is unlimited and never deducts from a balance.

function defaultLeaveBalance() {

    return {

        Casual: { quota: 12, used: 0 },

        Sick: { quota: 10, used: 0 },

        Earned: { quota: 15, used: 0 }
...
function getLeaveBalances() {
    ...
    return JSON.parse(
        localStorage.getItem("leaveBalances")
    ) || {};
...
// Returns an employee's balance, merged with defaults so newly
// added leave types never come back undefined.

function getEmployeeLeaveBalance(employeeId) {

    const balances = getLeaveBalances();
    const defaults = defaultLeaveBalance();

    const stored = balances[employeeId] || {};
